class GlslPipeline {
  constructor(renderer, uniforms = {}) {
    if (!renderer.capabilities.floatFragmentTextures) throw new Error('No OES_texture_float support for float textures.');
    ...
    this.uniforms.u_camera = { value: new THREE.Vector3() };
    ...
    this.billboard_scene = new THREE.Scene();
    this.billboard_camera = new THREE.Camera();
    ...
    this.lastTime = 0.0;
    this.time = 0.0;
    this.resolution = new THREE.Vector2(renderer.domElement.width, renderer.domElement.height);
  ...
  getBufferSize(name) {
    if (this.frag_src == null) return { width: 1.0, height: 1.0 };

    const size_exp = new RegExp(`uniform\\s*sampler2D\\s*${name}\\;\\s*\\/\\/*\\s(\\d+)x(\\d+)`, 'gm');
    const size_found = size_exp.exec(this.frag_src);
    if (size_found) return { width: parseInt(size_found[1]), height: parseInt(size_found[2]) };

    const scale_exp = new RegExp(`uniform\\s*sampler2D\\s*${name}\\;\\s*\\/\\/*\\s(\\d*\\.\\d+|\\d+)`, 'gm');
    const scale_found = scale_exp.exec(this.frag_src);
    if (scale_found) {
      if (scale_found.length > 2) return { width: parseFloat(scale_found[1]), height: parseFloat(scale_found[2]) };
      else if (scale_found.length > 1) return { width: parseFloat(scale_found[1]), height: parseFloat(scale_found[1]) };
    }
  ...
  createRenderTarget(b) {
    ...
    if (this.renderer.capabilities.isWebGL2 === false) type = THREE.HalfFloatType;
    ...
    let depth = null;
    if (b.depth) depth = new THREE.DepthTexture();

    let renderTarget = new THREE.WebGLRenderTarget(Math.floor(w), Math.floor(h), {
      wrapS: b.wrapS,
      wrapT: b.wrapT,
  ...
  updateUniforms(camera = null) {
    ...
    if (this.light) {
      ...
      if (this.light.shadow.map) this.uniforms.u_lightShadowMap = { value: this.light.shadow.map.texture };
    }
  ...
  updateBuffers() {
    ...
    for (let i = 0, il = this.buffers.length; i < il; i++) {
      ...
      if (b.width <= 1.0 && b.height <= 1.0)
        this.uniforms.u_resolution.value = new THREE.Vector2(Math.floor(this.resolution.x * b.width), Math.floor(this.resolution.y * b.height));
      else this.uniforms.u_resolution.value = new THREE.Vector2(b.width, b.height);
    ...
    for (let i = 0, il = this.doubleBuffers.length; i < il; i++) {
      ...
      if (db.width <= 1.0 && db.height <= 1.0)
        this.uniforms.u_resolution.value = new THREE.Vector2(Math.floor(this.resolution.x * db.width), Math.floor(this.resolution.y * db.height));
      else this.uniforms.u_resolution.value = new THREE.Vector2(db.width, db.height);
  ...
  setSize(width, height) {
    ...
    for (let i = 0; i < this.buffers.length; i++) {
      let b = this.buffers[i];
      if (b.width <= 1.0 && b.height <= 1.0) b.renderTarget.setSize(b.width * width, b.height * height);
    }
